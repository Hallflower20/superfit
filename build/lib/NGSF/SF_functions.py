...
def process_wrapper(args, int_obj, sn_spec_files, templates_sn_trunc_dict, 
                   templates_gal_trunc, templates_gal_trunc_dict, alam_dict,
                   lam, resolution, iterations, **kwargs):
    z, ext = args
    a, _ = core(
        int_obj,
        z,
...
def all_parameter_space(
    ...
    **kwargs
    ...
    wrapped = partial(
        process_wrapper,
        int_obj=int_obj,
        sn_spec_files=sn_spec_files,
        templates_sn_trunc_dict=templates_sn_trunc_dict,
        templates_gal_trunc=templates_gal_trunc,
        templates_gal_trunc_dict=templates_gal_trunc_dict,
        alam_dict=alam_dict,
        lam=lam,
    ...
    with mp.Pool(processes=cores) as pool:
        results = list(tqdm(pool.imap(wrapped, params), total=len(params)))
